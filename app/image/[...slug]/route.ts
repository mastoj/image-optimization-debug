...
export async function GET(request: NextRequest) {
  ...
  const mediaBaseUrl = "https://media.elkjop.com/assets";
  const pathname = request.nextUrl.pathname;
  const [_, assetType, blobId, ...rest] = pathname.split("/");
  const mediaUrl = `${mediaBaseUrl}/${assetType}/${blobId}?bust=true`;
  const urlEncodedMediaUrl = encodeURIComponent(mediaUrl);
  let url = `${request.nextUrl.origin}/_next/image?url=${urlEncodedMediaUrl}&w=${size}&q=${quality}`;
  if (bypassKey) {
